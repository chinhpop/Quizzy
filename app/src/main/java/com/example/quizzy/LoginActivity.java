        ...
        firebaseAuth.signInWithCredential(credential)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        if (user != null) {
                            // User is authenticated, now save data to Firestore
                            String userId = user.getUid();
                            String userName = user.getDisplayName(); // Get name from Google Account
                            String userEmail = user.getEmail(); // Get email from Google Account
                            ...
                            // Use set() with SetOptions.merge() to create the user document
                            // This won't overwrite existing data if the user logs in again
                            userRef.set(userData, SetOptions.merge())
                                    .addOnSuccessListener(aVoid -> {
                                        // Successfully saved data, now navigate to home
                                        setButtonsEnabled(true);
                                        isLoading = false;
                                    ...
                                    .addOnFailureListener(e -> {
                                        // Handle the error of failing to save to Firestore
                                        setButtonsEnabled(true);
                                        isLoading = false;
                                        showToast("Failed to save user data: " + e.getMessage(), Toast.LENGTH_LONG);
                                    });
                        }
                    } else {
